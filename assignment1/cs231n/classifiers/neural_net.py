...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    #############################################################################
    h1 = np.dot(X, W1) + b1
    h1_nl = np.maximum(0, h1)
    h2 = np.dot(h1_nl, W2) + b2
    scores = h2
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    ...
    # classifier loss.                                                          #
    #############################################################################
    scores_stabilized = scores - h1.max(axis=1, keepdims=True)
    scores_exp = np.exp(scores_stabilized)
    scores_sum = np.sum(scores_exp, axis=1, keepdims=True)
    scores_ratios = scores_exp / scores_sum
    losses = -np.log(scores_ratios[np.arange(N), y])
    loss = np.sum(losses)
  ...
  def predict(self, X):
    ...
    h1_nl = np.maximum(0, h1)
    scores = np.dot(h1_nl, self.params['W2']) + self.params['b2']
